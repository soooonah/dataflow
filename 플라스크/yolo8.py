...
import cv2
clsa={0: 'block', 1: 'circle', 2: 'cut', 3: 'dot', 
      4: 'empty', 5: 'long', 6: 'round', 
      7: 'slope', 8: 'square', 9: 'thickness'}
# 폴더 안의 파일 삭제 함수
...
def detect():
    ...
    if request.method == "POST":
        ...
        for file in file:
            ...
            img.save(f"static/bef/{inum}{filename}")
            
            print('원본 저장')
            
            model = YOLO('C:/Users/human/Desktop/학원/flask/best.pt')
            results = model(img)
            print("보기1",results)
            for i in range(len(results)):
                img1=results[i].plot()
                cv2.imwrite((f'static/aft/{inum}{filename}'), img1)
                #탐지된 객체의 이름을 가져옴
        ...
        print('이비지',firstimage)
        return render_template('imageshow.html', 
            files=files, resultlist=resultlist, pf=pf, 
            datanum=datanum, rate=rate, correct=correct,
            firstimage=firstimage, enumerate=enumerate, len=len,
